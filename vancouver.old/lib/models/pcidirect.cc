      ...
      for (unsigned i=0; i < count_bars(); i++)
	{
	  ...
	  if (bar)
		...
		_mb.bus_hostop.send(msg);
	      ...
	      {
		unsigned value = bar & ~0x1f | Cpu::bsr((~_masks[i] | 0xf) + 1);
		MessageHostOp msg(MessageHostOp::OP_ALLOC_IOMEM_REGION, value);
		if (_mb.bus_hostop.send(msg) && msg.ptr)
		  _bars[i] = reinterpret_cast<unsigned long>(msg.ptr) + (bar & 0x10);
		else
		  Logging::panic("can not map IOMEM region %x", value);
	      }
