...
cp $BINARIES/linux/initramfs $ISO_DIR/linux/initrd || exit 4

# ISOLINUX config

#SARGS="S0_DEFAULT hostvga:0x40000,0x40000 hostkeyb:0,0x60,1,12 tracebuffer:10240 vdisk:rom:///linux/grml.iso hostne2k "
SARGS_K="S0_DEFAULT kiosk_mode hostvga:0x40000,0x40000 hostkeyb:0,0x60,1,12,1,1 tracebuffer:10240 vdisk:rom:///linux/grml.iso service_config hostserial script_start:1 script_wait:5000 script_start:2"

BINARIES=" /nul/cycleburner_nul.gz  --- /nul/rocknshine_nul.gz --- /nul/eurosys.slides  --- /nul/vancouver_nul.gz --- /nul/remote_config_nul.gz --- /tools/munich --- /linux/bzImage --- /linux/initrd  --- /pistachio/i686_kernel  --- /pistachio/kickstart  --- /pistachio/sigma0  --- /pistachio/pingpong  --- /fiasco/fiasco  --- /fiasco/bootstrap  --- /fiasco/pingpong  --- /fiasco/roottask  --- /fiasco/sigma0 --- /linux.nulconfig  --- /pistachio/pistachio.template  --- /fiasco/fiasco.template  --- /cycleburner.template --- /rocknshine.template --- /empty.nulconfig sigma0::mem:12 name::/s0/timer name::/s0/config name::/s0/fs/rom || rom:///nul/remote_config_nul.gz linux:rom:///linux.nulconfig fiasco:rom:///fiasco/fiasco.template pistachio:rom:///pistachio/pistachio.template cycleburner:rom:///cycleburner.template"
