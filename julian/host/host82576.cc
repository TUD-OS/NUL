...
private:
  ...
  void handle_vf_message(unsigned vf_no)
  {
	...
	vf_set_mac(vf_no, vf_mac);
      ...
      break;
    case VF_PROMISCUOUS:
      // This message should only be generated by our software switch.
      _hwreg[VMOLR0 + vf_no] |= VMOLR_ROPE;
      _hwreg[pfmbxmem] = VF_PROMISCUOUS | CMD_ACK;
      break;
    default:
...
public:
    ...
    : PciDriver(clock, ALL & ~IRQ, bdf), _bus_hostop(bus_hostop)
  {
    ...
    // Enable default pool.
    _hwreg[VT_CTL] = VT_CTL_REP_ENABLE;
    Logging::printf("Default VF%u is the default VF.\n", (_hwreg[VT_CTL] >> 7) & 0x7);
