...
public:
  ...
  bool receive(MessageVirtualNetPing &msg)
  {
    ...
    while (_net.poll_receive(&data, &len)) {
      Logging::printf("Received %p+%x\n", data, len);
      receive_packet(data, len);
      ...
      _tx_done(*this)
  {
    ...
    uint8 *buffer = _net_mem.allocate<uint8>(4096);
    _net.queue_buffer(buffer, 2048);
    _net.queue_buffer(buffer + 2048, 2048);
  }
};
