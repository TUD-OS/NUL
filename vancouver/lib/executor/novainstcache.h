...
public:
  ...
  int get_instruction(MessageExecutor &msg, InstructionCacheEntry *&entry)
  {
	...
	//Logging::printf("as %x ar %x\n", entry->address_size, entry->cs_ar);
	...
	if (msg.vcpu->fault > 0)
	...
	else
	  {
		...
		msg.cpu->head.pid = 2;
	      ...
	      {
		if ((msg.cpu->inj_info & msg.vcpu->fault & 0x80000700) == 0x80000300)
		  if ((0x3c01 & (1 << (msg.cpu->inj_info & 0xff))) && (0x3c01 & (1 << (msg.vcpu->fault & 0xff)))
		      || (msg.cpu->inj_info == 0x80000b0e && (0x7c01 & (1 << (msg.vcpu->fault & 0xff)))))
		    {
...
public:
  ...
  InstructionCache(Motherboard &mb) : MemTlb(mb) {  memset(_values, 0, sizeof(*_values) * SIZE * ASSOZ); }
};
