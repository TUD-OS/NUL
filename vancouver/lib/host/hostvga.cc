...
public:
  bool  receive(MessageTimeout &msg)
  {
	...
	if (mode != _active_mode)
	  {
	    if (!_active_mode) memcpy(_saved, _backend, 1<<BACKEND_ORDER);
	    ...
	    MessageHostOp msg1(MessageHostOp::OP_ALLOC_IOMEM_REGION, (_modeinfo.physbase & ~0xfff) | 22);
	    if (!_mb.bus_hostop.send(msg1)) Logging::panic("can not get the framebuffer");
	    _graphic_ptr = msg1.ptr + (_modeinfo.physbase  & 0xfff);
	    _active_mode = mode;
	    if (!_active_mode) memcpy(_backend, _saved, 1<<BACKEND_ORDER);
	  }
...
PARAM(hostvga,
	...
	MessageHostOp msg(MessageHostOp::OP_ALLOC_IOMEM_REGION, 0xb8000 | HostVga::BACKEND_ORDER);
