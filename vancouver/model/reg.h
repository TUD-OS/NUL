...
#define REG_RW(NAME, OFFSET, VALUE, MASK) REG(NAME, OFFSET, unsigned NAME;, value = NAME; , if (!MASK) return false; NAME = (NAME & ~MASK) | (value & MASK); , NAME=VALUE;)
#define REG_WR(NAME, OFFSET, VALUE, MASK, RW1S, RW1C, WRITE_CALLBACK) REG(NAME, OFFSET, unsigned NAME; , value = NAME; ,  if (!MASK) return false; unsigned oldvalue = NAME; value = value & ~RW1S | ( value | oldvalue) & RW1S; value = value & ~RW1C | (~value & oldvalue) & RW1C; NAME = (NAME & ~MASK) | (value & MASK); WRITE_CALLBACK , NAME = VALUE;)
...
#undef  REG
#undef  REGSET
#define REGSET(NAME, ...)  public: bool NAME##_read(unsigned offset, unsigned &value) { switch (offset) { __VA_ARGS__ default: break; } return false; }
