...
struct MessageQueueOp
{
  ...
  union {
    struct {
      ...
      unsigned queue_len;	// Bytes
      unsigned queue_type;
      
      void    *context;		// Stores state of the queue. Must include tail pointer.
      unsigned context_len;	// Bytes

      unsigned irq_sem;		// OUTPUT: Client waits on this semaphore for packet processing to complete.
  ...
  MessageQueueOp(unsigned vnet, void *queue, unsigned queue_len, unsigned queue_type,
		 void *context, unsigned context_len) :
    op(ANNOUNCE), vnet(vnet), queue(queue), queue_len(queue_len), queue_type(queue_type),
    context(context), context_len(context_len)
  {};

  explicit
  MessageQueueOp(unsigned long long mac) :
    op(SET_MAC), mac(mac)
  {};
};
