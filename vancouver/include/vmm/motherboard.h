...
class Motherboard : public StaticReceiver<Motherboard>
{
 ...
 private:
  VirtualCpuState _cpustate[MAX_CPUS];
  Clock *_clock;
 public:
  ...
  VirtualCpuState *vcpustate(unsigned vcpunr) { assert(vcpunr < MAX_CPUS); return _cpustate + vcpunr; }
  ...
  void *operator new(size_t size)
  {
    ...
    Logging::printf("Motherboard @%p fpu[0] at %p\n", res, reinterpret_cast<Motherboard *>(res)->_cpustate[0].fpustate);
    return res;
  }
