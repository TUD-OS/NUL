...
class HostIde : public StaticReceiver<HostIde>
{
  ...
  {
    // select drive
    unsigned char packet_select[] = { 0x40, slave ? 0xb0 : 0xa0};
	...
	for (unsigned bdf, num = 0; bdf = pci.search_device(0x1, 0x1, num++);)
	  {
		...
		for (unsigned j=0; j < dev->disk_count(); j++)  mb.bus_disk.add(dev, &HostIde::receive_static<MessageDisk>, mb.bus_disk.count());
	      }
	  }
      },
