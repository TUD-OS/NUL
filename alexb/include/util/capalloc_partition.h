...
public:
  CapAllocatorAtomicPartition(unsigned long __cap_start = ~0UL, unsigned divider = 1) //~0UL means disabled
     : CapAllocatorAtomic<BITS, error_doublefree>(__cap_start), _divider(divider) {}
  ...
  unsigned alloc_cap(unsigned count = 1, unsigned cpu = ~0U) {
    ...
    unsigned res = CapAllocatorAtomic<BITS, error_doublefree>::__alloc_cap(count, start);
