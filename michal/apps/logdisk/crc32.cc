...
uint32_t calc_crc(uint32_t initial, const uint8_t *buf, uint32_t size)
{
	...
	const uint32_t *start = reinterpret_cast<const uint32_t *>(buf);
	const uint32_t *end = reinterpret_cast<const uint32_t *>(buf + (size & 0xfffffffc));
	...
	buf = reinterpret_cast<const uint8_t *>(start);
	size = size & 0x3;
	while (size--) {
		crc = crc ^ *buf++;
		crc = crctab[crc & 0xff] ^ crc >> 8;
