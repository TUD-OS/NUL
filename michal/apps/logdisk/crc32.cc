...
uint32 calc_crc(uint32 initial, const uint8 *buf, uint32 size)
{
	...
	const uint32 *start = reinterpret_cast<const uint32 *>(buf);
	const uint32 *end = reinterpret_cast<const uint32 *>(buf + (size & 0xfffffffc));
	uint32 crc = initial;
	...
	buf = reinterpret_cast<const uint8 *>(start);
	size = size & 0x3;
	while (size--) {
		crc = crc ^ *buf++;
		crc = crctab[crc & 0xff] ^ crc >> 8;
